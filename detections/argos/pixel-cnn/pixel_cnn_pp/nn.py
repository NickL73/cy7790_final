...
def sample_from_discretized_mix_logistic(l,nr_mix):
    ...
    sel = tf.one_hot(tf.argmax(logit_probs - tf.log(-tf.log(tf.random_uniform(logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5,seed=1))), 3), depth=nr_mix, dtype=tf.float32)
    ...
    u = tf.random_uniform(means.get_shape(), minval=1e-5, maxval=1. - 1e-5,seed=1)
