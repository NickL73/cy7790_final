...
def get_Mahalanobis_score(model, test_loader, num_classes, outf, out_flag, net_type, sample_mean, precision, layer_index, magnitude):
    ...
    for data, target in test_loader:
        ...
        if net_type == 'densenet':
            gradient.index_copy_(1, torch.LongTensor([0]), gradient.index_select(1, torch.LongTensor([0])) / (63.0/255.0))
            gradient.index_copy_(1, torch.LongTensor([1]), gradient.index_select(1, torch.LongTensor([1])) / (62.1/255.0))
            gradient.index_copy_(1, torch.LongTensor([2]), gradient.index_select(1, torch.LongTensor([2])) / (66.7/255.0))
...
def get_LID(model, test_clean_data, test_adv_data, test_noisy_data, test_label, num_output):
    ...
    for data_index in range(int(np.floor(test_clean_data.size(0)/batch_size))):
        data = test_clean_data[total : total + batch_size]
        adv_data = test_adv_data[total : total + batch_size]
        noisy_data = test_noisy_data[total : total + batch_size]
        target = test_label[total : total + batch_size]
