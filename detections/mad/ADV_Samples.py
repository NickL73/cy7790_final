...
def main():
    ...
    if args.net_type == 'densenet':
        ...
        model.load_state_dict(torch.load(pre_trained_net, map_location =torch.device('cpu')))
        in_transform = transforms.Compose([transforms.ToTensor(), \
    ...
    for data, target in test_loader:
        ...
        noisy_data = torch.add(data.data, random_noise_size, torch.randn(data.size())) 
        ...
        if args.adv_type == 'FGSM': 
            ...
            if args.net_type == 'densenet':
                ...
                gradient.index_copy_(1, torch.LongTensor([0]), \
                                     gradient.index_select(1, torch.LongTensor([0])) / (0.2023))
                gradient.index_copy_(1, torch.LongTensor([1]), \
                                     gradient.index_select(1, torch.LongTensor([1])) / (0.1994))
                gradient.index_copy_(1, torch.LongTensor([2]), \
                                     gradient.index_select(1, torch.LongTensor([2])) / (0.2010))
